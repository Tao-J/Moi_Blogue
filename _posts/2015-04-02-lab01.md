...
layout: post
title: lab01
title_real: Acadia board hands on
low: low
---
#lab01
##1. 开发板连接示意图
![](./Embeded_diagram_lab01.png)
...
由于IP绑定了MAC，所以得克隆MAC地址。这里隐去真实地址。
...
配置DNS服务，直接修改Debian发行版提供的`/etc/network/interfaces`就行。
同时一并把静态IP也写进去了。
...
`ssh`到who.tao-j.com所对应的服务器上，回`ping`这台设备。其实不用这步也已经证明了两者的互通。
...
##5. 看下SSH配置文件
准确说应该是sshd配置文件`/etc/ssh/sshd_config` 

```
root@Acadia:/etc/ssh# vim sshd_config
```

以下配置打开端口22，使用指定的key。其实这一开始就配好了。
...
###两种最基本的SSH登入方式
1.直接连接该板配好的静态IP，会跳出请求输入密码。板子一开始得用passwd设置一个密码，空密码没发登入。
...
2.使用rsa-key pair。
首先在客户机上生成一个rsa keypair，并将public key填写到开发板相应用户home folder的`.ssh/authorized_keys`中。
...
##6. 查看登入用户
一个用户是串口，一个用户从ssh登入，还有一个bash，应该是mux到了ttymxc0。
